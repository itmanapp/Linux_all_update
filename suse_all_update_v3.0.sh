...
usage() {
cat <<'USAGE'
suse_all_update_v3.0.sh — openSUSE / SUSE 系統更新腳本

用法：
  ./suse_all_update_v3.0.sh [選項]
...
範例：
  ./suse_all_update_v3.0.sh --dry-run
  ./suse_all_update_v3.0.sh --only zypper
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            ;;
        --version)
            echo "suse_all_update_v3.0.sh $VERSION"
            exit 0
            ;;
        -n|--dry-run)
            DRY_RUN=true
...
if ! command -v zypper >/dev/null 2>&1; then
    ...
    echo "   Ubuntu／Debian 請改用 ubuntu_all_update_v2.1.sh"
    exit 1
fi
...
# --------- 檢查是否有殘留的 zypper 鎖檔 ----------
#
# v2.1：不再建議使用者手動刪鎖檔。zypper 自己會處理殘留的鎖；
...
# --------- 本次更新的套件清單 ----------
# 以「升級前後的已安裝套件快照」求差異，比解析 zypper 的表格輸出可靠。
...
# 模擬模式：zypper 的輸出格式依版本而異，這裡只做寬鬆解析，
# 抓「are going to be ...」之後的縮排區塊並取出套件名稱。
# 這只是預覽；解析不到時會顯示「本次無任何套件更新」。
pkg_simulated_lines() {
    local log="$1"
    [[ -r "$log" ]] || return 0
    awk '
        /are going to be (upgraded|installed|downgraded|removed|changed)/ { grab = 1; next }
        grab && /^[[:space:]]/ {
            gsub(/^[[:space:]]+/, "")
            n = split($0, a, /[[:space:]]+/)
            for (i = 1; i <= n; i++) {
                ...
                if (t == "") continue
                if (t ~ /[0-9]/) continue
                if (t ~ /^(->|to|from|and|The)$/) continue
                if (t !~ /^[A-Za-z_][A-Za-z0-9._+-]*$/) continue
                printf "%s\t\t\n", t
            }
            next
        }
...
if [[ "$ONLY_ZYPPER" != true ]]; then
    ...
    if [[ "$DRY_RUN" == true ]]; then
        # 一定要帶 LC_ALL=C：log 的錯誤／警告判定依賴英文輸出。
        ZYPPER_UPGRADE_CMD_ARR=(env LC_ALL=C zypper --non-interactive "$ZYPPER_UPGRADE_CMD" --dry-run)
    ...
    # --------- refresh ----------
    #
    # v2.1：zypper refresh 的 106（ZYPPER_EXIT_INF_REPOS_SKIPPED）是資訊性狀態，
    ...
    # 算出本次的套件異動清單
    if [[ "$DRY_RUN" == true ]]; then
        PKG_UPDATED_LINES="$(pkg_simulated_lines "$ZYPPER_UPGRADE_LOG")"
        PKG_DELTA_OK=true
    else
    ...
    # --------- 檢查 zypper 的 Error（v2.1：不再把每個 Warning 都當成問題） ----------
...
# v2.1：不再丟棄非 0／102 的結果，才不會把「檢查失敗」誤認為「不需要重開機」。
if command -v zypper >/dev/null 2>&1; then
