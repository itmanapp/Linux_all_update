#!/bin/bash
# ============================================================
#  ubuntu_all_update_v2.0.sh
#
#  Ubuntu / Debian 系統更新腳本（apt + snap + flatpak）
#
#  這是 v2.0，與 v1 的 ubuntu_all_update.sh 並存，不會覆蓋它。
#  v2.0 修正了 v1 的安全性與正確性問題，並新增命令列選項。
...
usage() {
cat <<'USAGE'
ubuntu_all_update_v2.0.sh — Ubuntu / Debian 系統更新腳本

用法：
  ./ubuntu_all_update_v2.0.sh [選項]
...
範例：
  ...
  ./ubuntu_all_update_v2.0.sh --only apt
  ./ubuntu_all_update_v2.0.sh -y --keep-logs 5
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            ;;
        --version)
            echo "ubuntu_all_update_v2.0.sh $VERSION"
            exit 0
            ;;
        -n|--dry-run)
            DRY_RUN=true
...
if [[ "$ONLY_APT" == true ]]; then
    for tool in apt-get dpkg; do
        if ! command -v "$tool" >/dev/null 2>&1; then
            ...
            echo "   openSUSE／SUSE 請改用 suse_all_update_v2.0.sh"
            exit 1
        fi
    done
fi
...
if [[ "$ONLY_APT" == true ]]; then
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        # v2.0：dpkg 處於半殘狀態是「真的問題」，要進 WARNINGS 並影響 exit code，
...
apt_lock_state() {
    ...
    for lockfile in /var/lib/dpkg/lock-frontend /var/lib/apt/lists/lock; do
        ...
        case "$rc" in
            ...
            *) return 2 ;;   # sudo 失敗／fuser 錯誤 → 無法判定
        esac
    done
...
if [[ "$ONLY_APT" != true ]]; then
    ...
    # --------- 檢查 APT 的 Error（v2.0：不再把每個 W: 都當成問題） ----------
