...
DPKG_STARTED=$SECONDS

if sudo DEBIAN_FRONTEND=noninteractive LC_ALL=C dpkg --configure -a >/dev/null 2>&1; then
echo -e "${GREEN}✓ 檢查完成${RESET}"
record_step "dpkg configure" "OK" "sudo dpkg --configure -a" "$((SECONDS - DPKG_STARTED))"
else
echo -e "${RED}✗ 檢查失敗（不影響後續更新）${RESET}"
record_step "dpkg configure" "FAIL" "sudo dpkg --configure -a" "$((SECONDS - DPKG_STARTED))" "dpkg --configure -a 失敗"
fi
...
if ensure_sudo && sudo apt-get autoclean >/dev/null 2>&1; then
    record_step "apt autoclean" "OK" "sudo apt-get autoclean" "$((SECONDS - CLEAN_STARTED))"
else
    record_step "apt autoclean" "FAIL" "sudo apt-get autoclean" "$((SECONDS - CLEAN_STARTED))" "清理快取失敗（不影響更新結果）"
fi
