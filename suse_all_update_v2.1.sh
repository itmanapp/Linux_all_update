#!/bin/bash
# ============================================================
#  suse_all_update_v2.1.sh
#
#  openSUSE / SUSE 系統更新腳本（zypper + flatpak）
#
#  這是 v2.1，與 v1 的 suse_all_update.sh 並存，不會覆蓋它。
...
usage() {
cat <<'USAGE'
suse_all_update_v2.1.sh — openSUSE / SUSE 系統更新腳本

用法：
  ./suse_all_update_v2.1.sh [選項]
...
範例：
  ./suse_all_update_v2.1.sh --dry-run
  ./suse_all_update_v2.1.sh --only zypper
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            ...
            ;;
        --version)
            echo "suse_all_update_v2.1.sh $VERSION"
            exit 0
            ;;
        -n|--dry-run)
            DRY_RUN=true
